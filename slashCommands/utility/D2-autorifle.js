  ...
  run: async (client, interaction) => {
    try {
      ...
      async function getRaidCompletions() {
        try {
          const response = await axios.get(`https://www.bungie.net/Platform/Destiny2/3/Account/4611686018470076418/Character/0/Stats/?groups=102&modes=4&periodType=AllTime`, {
            headers: {
              'X-API-Key': API_KEY
            }
          });
          if (response.data.ErrorCode === 1) {
            interaction.reply(`Autorifle: ${response.data.Response.raid.allTime.weaponKillsAutoRifle.basic.value.toString()}`);
          } else {
